...
| 種類 | 規則 | 例 |
|------|------|-----|
| クラス名 | PascalCase | UserController |
| メソッド名 | camelCase | getUserById |
| 変数名 | camelCase | userName |
| 定数 | UPPER_SNAKE_CASE | MAX_TAG_COUNT |
| パッケージ名 | lowercase | com.javatraining.backend |

#### 命名規則の説明と例

```java
// camelCase: 最初は小文字、その後の単語は大文字で始める
String userName = "田中太郎";
int maxRetryCount = 3;
List<UserTag> activeUserTags = new ArrayList<>();

// PascalCase: すべての単語を大文字で始める  
...
private static final List<String> AVAILABLE_CATEGORIES = Arrays.asList("Level 0", "Level 1");
```
...
public class User {
    ...
    /**
     * ページ内の全ユーザーのタグをバッチで取得（N+1問題解決）
     * 複数のユーザーのタグを1回のクエリで効率的に取得する
     * 
     * @param users ユーザーリスト
     */
    private void loadUserTagsBatch(List<User> users) {
...
public class UserService {
    ...
    public User createUser(User user) {
        try {
            ...
            // その他のエラーはログ出力後にRuntimeExceptionで包んで再スロー
...
public class UserController {
    ...
    public String registerUser(@Valid @ModelAttribute User user,
        ...
        try {
            ...
            // 成功メッセージの設定
            redirectAttributes.addFlashAttribute("success", "ユーザーを登録しました。");
            return "redirect:/users/" + savedUser.getId();
            
        } catch (IllegalArgumentException e) {
            // ビジネスルールエラー：エラーメッセージを表示して入力画面に戻る
            model.addAttribute("error", e.getMessage());
            addFormAttributes(model);
            return "users/registration";
            
        } catch (Exception e) {
            // システムエラー：ログ出力後にエラーメッセージを表示
...
- [ ] トランザクション管理は適切か
- [ ] nullチェックを実装したか
- [ ] このガイドのセクション6（命名規則・コメント・エラーハンドリング、N+1等）に従っているか
